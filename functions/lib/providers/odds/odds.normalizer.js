...
exports.OddsNormalizer = void 0;
const bookmaker_registry_js_1 = require("./bookmaker.registry.js");
class OddsNormalizer {
    /**
     * Valida si una cuota decimal es válida (> 1.00, número finito y no NaN)
     */
    static isValidOdds(price) {
     ...
     */
    static normalizeEventOdds(event, provider = 'the-odds-api') {
        const items = [];
        const fetchedAt = new Date().toISOString();
        for (const b of event.bookmakers || []) {
            const internalBookmakerId = bookmaker_registry_js_1.BookmakerRegistry.mapProviderKeyToInternalId(b.key);
            const bookmakerDef = bookmaker_registry_js_1.BookmakerRegistry.getBookmaker(internalBookmakerId);
            for (const m of b.markets || []) {
                ...
                for (const outcome of m.outcomes || []) {
                    if (!this.isValidOdds(outcome.price))
                        continue;
                    let selection = null;
     ...
     */
    static buildOddsComparison(event, targetBookmakerIds = ['pinnacle', 'bet365', 'betfair', '1xbet', 'betplay', 'wplay']) {
        ...
        for (const sel of selectionKeys) {
            ...
            for (const bkId of targetBookmakerIds) {
                const bkDef = bookmaker_registry_js_1.BookmakerRegistry.getBookmaker(bkId);
                // 1. Si la casa no está soportada por el proveedor, marcar explícitamente sin inventar datos
                if (!bookmaker_registry_js_1.BookmakerRegistry.isSupported(bkId)) {
                    rows.push({
                        bookmakerId: bkId,
                ...
                const matchingItem = normalizedItems.find(item => item.bookmakerId === bkId &&
            ...
            const validPrices = validRows.map(r => r.price);
            let bestOdds = null;
            let worstOdds = null;
            let averageOdds = 0;
            let medianOdds = 0;
...
exports.OddsNormalizer = OddsNormalizer;
//# sourceMappingURL=odds.normalizer.js.map