...
class CorrelationDetector {
     ...
     */
    static evaluateSameMatchPair(a, b) {
        const notes = [];
        ...
        if (isTotA && isTotB) {
            if ((a.market.startsWith('over_') && b.market.startsWith('under_')) ||
        ...
        if ((a.market === 'btts' && a.selection === 'yes' && b.market === 'under_2.5' && b.selection === 'under') ||
            ...
            notes.push(`Correlación restrictiva: Menos de 2.5 y Ambos Equipos Anotan (Sí) solo permite un resultado (1-1). Cuota altamente dependiente.`);
            return {
                isExcluded: false,
                hasCorrelation: true,
                correlationRisk: 'HIGH',
        ...
        if (hasOver25 && hasBttsYes) {
            notes.push(`Correlación intra-partido positiva: Más de 2.5 Goles y Ambos Equipos Anotan (Sí) dependen del mismo flujo ofensivo. Las casas suelen ajustar cuotas en combinadas del mismo evento.`);
            return {
                isExcluded: false,
                hasCorrelation: true,
                correlationRisk: 'HIGH',
        ...
        if (hasUnder25 && hasBttsNo) {
            notes.push(`Correlación intra-partido positiva: Menos de 2.5 Goles y Ambos Equipos NO Anotan (5 de los 6 marcadores under coinciden con BTTS No).`);
            return {
                isExcluded: false,
                hasCorrelation: true,
                correlationRisk: 'HIGH',
        ...
        if ((a.market === '1X2' && a.selection === 'home' && b.market === 'double_chance' && b.selection === 'home_draw') ||
        ...
        if ((a.market === '1X2' && a.selection === 'home' && b.market === 'draw_no_bet' && b.selection === 'home_dnb') ||
        ...
        if (is1X2 && isSpreadOrTotal) {
            notes.push(`Correlación intra-partido moderada: Victoria de equipo y mercado de totales en el mismo partido (${a.matchDescription}).`);
            return {
                isExcluded: false,
                hasCorrelation: true,
        ...
        notes.push(`Múltiples selecciones en el mismo partido (${a.matchDescription}). Riesgo de correlación bajo-moderado.`);
        return {
            isExcluded: false,
            hasCorrelation: true,
            correlationRisk: 'LOW',
    ...
    static escalateRisk(current, candidate) {
        const levels = {
            NONE: 0,
            LOW: 1,
            MEDIUM: 2,
...
exports.CorrelationDetector = CorrelationDetector;
