...
class ParlayService {
    ...
    async generateAndStoreUserParlays(userId, forcedDate) {
        ...
        const engineResult = parlay_engine_js_1.ParlayEngine.generateForUser(selections, userPrefs);
        // 4. Transformar y explicar los finalistas seleccionados
        const savedParlays = [];
        // Foco del Día
        if (engineResult.focoDelDia) {
            const explained = await this.buildSavedParlay(userId, date, engineResult.focoDelDia, 'FOCO_DEL_DIA', 'Foco del Día');
            savedParlays.push(explained);
        }
        // Alta Probabilidad
        if (engineResult.altaProbabilidad) {
            const explained = await this.buildSavedParlay(userId, date, engineResult.altaProbabilidad, 'ALTA_PROBABILIDAD', 'Parley Alta Probabilidad');
            savedParlays.push(explained);
        }
        // De Valor
        if (engineResult.deValor) {
            const explained = await this.buildSavedParlay(userId, date, engineResult.deValor, 'VALOR', 'Parley de Valor (+EV)');
            savedParlays.push(explained);
        }
        // Alternativas
        for (let i = 0; i < engineResult.alternativas.length; i++) {
            const alt = engineResult.alternativas[i];
            const explained = await this.buildSavedParlay(userId, date, alt, 'ALTERNATIVAS', `Alternativa ${i + 1}`);
            savedParlays.push(explained);
        }
        // 5. Persistir en Firestore (parlays/{parlayId})
        ...
        try {
            ...
            if (snapshot.empty) {
                ...
                const recentSnap = await this.db
                    ...
                    .limit(10)
                    .get();
                return recentSnap.docs.map(doc => doc.data());
            }
            return snapshot.docs.map(doc => doc.data());
        }
        catch (error) {
            logger.error(`Error recuperando parlays guardados para ${userId}`, error);
            return [];
        }
    }
    /**
     ...
     */
    async getAvailableSelections(userPrefs) {
        try {
     ...
     */
    generateFallbackSelections(userPrefs) {
...
exports.ParlayService = ParlayService;
//# sourceMappingURL=parlay.service.js.map