...
const gemini_context_provider_js_1 = require("../providers/ai/gemini.context.provider.js");
const mock_context_provider_js_1 = require("../providers/ai/mock.context.provider.js");
...
class ContextAnalysisService {
    ...
    constructor(provider) {
        ...
        else if (index_js_1.config.isMockMode || !index_js_1.config.apiKeys.gemini) {
            this.provider = new mock_context_provider_js_1.MockContextProvider();
        }
        else {
            this.provider = new gemini_context_provider_js_1.GeminiContextProvider(index_js_1.config.apiKeys.gemini, index_js_1.config.geminiModel);
        }
     ...
     */
    async analyzeTopCandidates(candidates, maxFinal = 8) {
    ...
    async analyzeWithCache(input) {
        ...
        if (stored?.output) {
            ...
            cache_service_js_1.globalCache.set(cacheKey, stored.output, index_js_1.config.cache.analysisTtlSeconds);
    ...
    async executeAnalysis(input, inputHash, cacheKey) {
        ...
        try {
            ...
            error = errMsg;
            logger.error(`Error en análisis contextual de ${input.matchId}. El motor estadístico continúa.`, err);
        ...
        const record = {
            ...
            inputHash,
            model: this.provider.isMock ? 'mock' : index_js_1.config.geminiModel,
            input,
            output,
            durationMs,
            error,
        ...
        if (output) {
            cache_service_js_1.globalCache.set(cacheKey, output, index_js_1.config.cache.analysisTtlSeconds);
        }
        return output;
    }
    // ---------------------------------------------------------------------------
     ...
     */
    hasSufficientData(input) {
        const hasForm = (input.homeRecentForm && input.homeRecentForm.length >= 3) ||
        ...
        const hasAnyContext = (input.relevantNews?.length ?? 0) > 0 ||
    ...
    // Firestore
    // ---------------------------------------------------------------------------
    async findStoredByHash(matchId, inputHash) {
        try {
     ...
     */
    async getStoredContextAnalyses(matchId) {
        try {
            const snap = await this.db
                .collection('aiAnalyses')
                .where('matchId', '==', matchId)
            ...
            logger.error(`Error recuperando análisis contextuales para ${matchId}`, error);
            return [];
        }
    }
}
exports.ContextAnalysisService = ContextAnalysisService;
//# sourceMappingURL=context.analysis.service.js.map