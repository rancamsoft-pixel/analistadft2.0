...
import { CorrelationDetector } from './correlation.detector.js';
import { DataQuality } from '../prediction/prediction.interface.js';

export interface ParlayEngineResult {
  focoDelDia: ParlayCandidate | null;
  altaProbabilidad: ParlayCandidate | null;
  deValor: ParlayCandidate | null;
  alternativas: ParlayCandidate[];
...
export class ParlayEngine {
   ...
   */
  static generateForUser(
    availableSelections: ParlaySelection[],
    userPrefs: UserPreferencesContext
  ): ParlayEngineResult {
    ...
    const highProbCandidates = this.generateCandidatesForType(
      validSelections,
      'HIGH_PROBABILITY_PARLAY'
    );

    const valueCandidates = this.generateCandidatesForType(
      validSelections,
      'VALUE_PARLAY'
    );

    const balancedCandidates = this.generateCandidatesForType(
      validSelections,
      'BALANCED_PARLAY'
    );

    const totalCandidatesCount =
      highProbCandidates.length + valueCandidates.length + balancedCandidates.length;
  ...
  static filterByUserPreferences(
    ...
    return selections.filter(sel => {
      ...
      if (userPrefs.activeBookmakerIds && userPrefs.activeBookmakerIds.length > 0) {
        const matchesBookmaker = userPrefs.activeBookmakerIds.some(
          bId => bId.toLowerCase() === sel.bookmakerId.toLowerCase() ||
      ...
      if (userPrefs.activeMarketKeys && userPrefs.activeMarketKeys.length > 0) {
        const matchesMarket = userPrefs.activeMarketKeys.some(mKey => {
          if (mKey === '1X2' && sel.market === '1X2') return true;
          if (mKey === 'over_under' && (sel.market.startsWith('over_') || sel.market.startsWith('under_'))) return true;
          if (mKey === 'both_teams_to_score' && sel.market === 'btts') return true;
          if (mKey === 'double_chance' && sel.market === 'double_chance') return true;
          if (mKey === 'draw_no_bet' && sel.market === 'draw_no_bet') return true;
   ...
   */
  static filterValidSelections(selections: ParlaySelection[]): ParlaySelection[] {
    return selections.filter(sel => {
      // Cuota válida y positiva
      if (!sel.odds || sel.odds <= 1.05 || !Number.isFinite(sel.odds)) return false;
      // Probabilidad calculada y en rango
      if (!sel.probability || sel.probability <= 0.05 || sel.probability >= 0.98) return false;
      // No permitir datos insuficientes
      if (sel.dataQuality === 'INSUFFICIENT') return false;
      return true;
    });
  }
   ...
   */
  private static generateCandidatesForType(
    pool: ParlaySelection[],
    type: ParlayType
  ): ParlayCandidate[] {
    const limits = PARLAY_LIMITS[type];
    ...
    for (let size = limits.minSelections; size <= limits.maxSelections; size++) {
      ...
      for (const combo of combos) {
        // Evaluar correlación
        const correlation = CorrelationDetector.evaluate(combo);
   ...
   */
  private static filterPoolForType(pool: ParlaySelection[], type: ParlayType): ParlaySelection[] {
    switch (type) {
   ...
   */
  private static resolveAggregatedQuality(qualities: DataQuality[]): DataQuality {
    if (qualities.includes('INSUFFICIENT')) return 'INSUFFICIENT';
   ...
   */
  private static selectFinalists(
    highProbCandidates: ParlayCandidate[],
    valueCandidates: ParlayCandidate[],
    balancedCandidates: ParlayCandidate[],
    allCount: number
  ): ParlayEngineResult {
    ...
    const allTop = [topHighProb, topValue, topBalanced].filter((c): c is ParlayCandidate => c !== null);
    let focoDelDia: ParlayCandidate | null = null;
    ...
    const alternativas: ParlayCandidate[] = [];
    const usedCombosKey = new Set<string>();
  ...
  private static getCombinationKey(selections: ParlaySelection[]): string {
    return selections
   ...
   */
  private static getCombinations<T>(arr: T[], k: number, maxCount: number = 100): T[][] {
    const results: T[][] = [];
