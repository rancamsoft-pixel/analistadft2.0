...
function generateDecayWeights(n: number, decayRate: number): number[] {
...
export function calculateFormScore(
  results: Array<'W' | 'D' | 'L'>,
  windowSize: 5 | 10
): { score: number; matchesUsed: number } {
  const window = results.slice(0, windowSize);
  if (window.length === 0) return { score: 0.5, matchesUsed: 0 }; // Neutral si no hay datos
...
export interface FormAdjustment {
  /** Multiplicador sobre λ_home: > 1 = local en mejor forma */
  homeLambdaMultiplier: number;
  /** Multiplicador sobre λ_away: > 1 = visitante en mejor forma */
 ...
 */
export function calculateFormAdjustment(
  ...
  const homeLambdaMultiplier = Math.max(0.82, Math.min(1.18,
    (0.85 + homeFormScore * 0.30) * homeGoalFactor
  ));
  const awayLambdaMultiplier = Math.max(0.82, Math.min(1.18,
    (0.85 + awayFormScore * 0.30) * awayGoalFactor
  ));
