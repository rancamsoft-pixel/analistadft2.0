...
import { InjuryData, MatchInputData, PredictionFactor, StandingData } from '../prediction.interface.js';
...
export interface HomeAwayAdjustment {
  homeLambdaMultiplier: number;
  awayLambdaMultiplier: number;
 ...
 */
export function calculateHomeAwayAdjustment(input: MatchInputData): HomeAwayAdjustment {
 ...
 */
function calculateStandingFactor(standings: StandingData): number {
  if (standings.totalTeams === 0) return 1.0;
 ...
 */
function calculateRestFactor(homeRestDays: number, awayRestDays: number): number {
  const diff = homeRestDays - awayRestDays;
  if (Math.abs(diff) < 1) return 1.0; // Sin diferencia significativa
 ...
 */
function calculateInjuryFactor(injuries: InjuryData): number {
