...
export class MockContextProvider implements ContextAnalysisProvider {
  readonly providerName = 'MockContextProvider';
  readonly isMock = true;

  async analyzeContext(input: ContextAnalysisInput): Promise<ContextAnalysisOutput> {
    const hasInjuries =
      (input.homeInjuries && input.homeInjuries.length > 0) ||
    ...
    if (input.homeRestDays !== undefined && input.awayRestDays !== undefined) {
      if (input.homeRestDays > input.awayRestDays + 1) {
        positiveFactors.push(
          `${input.homeTeam} tiene ${input.homeRestDays} días de descanso vs ${input.awayRestDays} del visitante`
        );
      } else if (input.awayRestDays > input.homeRestDays + 1) {
        negativeFactors.push(
          `${input.awayTeam} llega con más descanso (${input.awayRestDays}d vs ${input.homeRestDays}d)`
        );
      }
    }
    ...
    if (input.homeStandingRank !== undefined && input.awayStandingRank !== undefined) {
      if (input.homeStandingRank < input.awayStandingRank) {
        positiveFactors.push(
          `${input.homeTeam} está mejor posicionado en la tabla (${input.homeStandingRank}° vs ${input.awayStandingRank}°)`
        );
      } else if (input.awayStandingRank < input.homeStandingRank) {
        negativeFactors.push(
          `${input.awayTeam} ocupa mejor posición en la tabla (${input.awayStandingRank}° vs ${input.homeStandingRank}°)`
        );
      }
    }
    ...
    const adjustment: ContextAnalysisOutput['confidenceAdjustment'] = 0;
    ...
    return {
      ...
      recommendationContext:
        `Análisis mock generado para ${input.matchId}. ` +
