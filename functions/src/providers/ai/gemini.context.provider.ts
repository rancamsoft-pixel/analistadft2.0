...
const logger = new StructuredLogger('GeminiContextProvider');
...
export class GeminiContextProvider implements ContextAnalysisProvider {
  readonly providerName = 'GeminiContextProvider';
  ...
  async analyzeContext(input: ContextAnalysisInput): Promise<ContextAnalysisOutput> {
  ...
  async callGeminiApi(input: ContextAnalysisInput): Promise<ContextAnalysisOutput> {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `[GeminiContextProvider] Error HTTP ${response.status} ${response.statusText}. ${errorText.slice(0, 200)}`
      );
    }
  ...
  private buildRequestBody(input: ContextAnalysisInput): object {
...
8. Responde ÚNICAMENTE con el JSON canónico especificado. Sin texto adicional, sin markdown.`;
  ...
  private buildUserPrompt(input: ContextAnalysisInput): string {
   ...
   */
  parseAndValidate(rawText: string, input: ContextAnalysisInput): ContextAnalysisOutput {
    let parsed: unknown;
   ...
   */
  private attemptJsonRepair(rawText: string): unknown {
   ...
   * - Arrays no nulos
   */
  private normalizeOutput(parsed: unknown, input: ContextAnalysisInput): ContextAnalysisOutput {
    const raw = parsed as Record<string, unknown>;

    // Validar confidenceAdjustment
    const rawAdj = raw['confidenceAdjustment'];
    const adj = Number(rawAdj);
    let confidenceAdjustment: ConfidenceAdjustment = 0;
    if (VALID_CONFIDENCE_ADJUSTMENTS.includes(adj as ConfidenceAdjustment)) {
      confidenceAdjustment = adj as ConfidenceAdjustment;
    } else {
    ...
    const relevantNews: NewsSource[] = (
      Array.isArray(raw['relevantNews']) ? raw['relevantNews'] as unknown[] : []
    )
      .filter((n): n is NewsSource => {
    ...
    const sourceQuality = validSourceQualities.includes(String(raw['sourceQuality']))
      ? (raw['sourceQuality'] as ContextAnalysisOutput['sourceQuality'])
      : 'LOW';
  ...
  private sanitizeStringArray(value: unknown): string[] {
    ...
    return value
      ...
      .map(item => this.sanitizeText(item as string));
  }
