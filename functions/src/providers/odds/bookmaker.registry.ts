export interface BookmakerDefinition {
  id: string;             // ID interno en la plataforma (ej. 'betplay', 'pinnacle')
  name: string;           // Nombre visible
  providerKey: string | null; // Key en The Odds API si está soportada, o null si no
  country: string;
  isAvailableInTheOddsApi: boolean;
}

export const BOOKMAKER_REGISTRY: Record<string, BookmakerDefinition> = {
  pinnacle: {
    id: 'pinnacle',
    name: 'Pinnacle',
...
export class BookmakerRegistry {
   ...
   */
  static getBookmaker(internalId: string): BookmakerDefinition {
    const key = internalId.toLowerCase();
    return BOOKMAKER_REGISTRY[key] || {
      id: internalId,
   ...
   */
  static getProviderKey(internalId: string): string | null {
   ...
   */
  static isSupported(internalId: string): boolean {
    const b = this.getBookmaker(internalId);
   ...
   */
  static mapProviderKeyToInternalId(providerKey: string): string {
    const cleanKey = providerKey.toLowerCase();
    for (const [internalId, def] of Object.entries(BOOKMAKER_REGISTRY)) {
   ...
   */
  static getAllKnown(): BookmakerDefinition[] {
    return Object.values(BOOKMAKER_REGISTRY);
  }
}
