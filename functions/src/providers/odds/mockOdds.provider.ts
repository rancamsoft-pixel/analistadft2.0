...
export const MOCK_EVENT_ODDS: EventOdds[] = [
  {
    id: 'match-101',
    sportKey: 'soccer_epl',
...
export class MockOddsProvider implements OddsProvider {
  readonly providerName = 'MockOddsProvider';
  ...
  async getUpcomingOdds(sportKey: string): Promise<EventOdds[]> {
    if (!sportKey || sportKey === 'all') {
      return Promise.resolve(MOCK_EVENT_ODDS);
    }
    return Promise.resolve(MOCK_EVENT_ODDS.filter(e => e.sportKey === sportKey));
  }

  async getMatchOdds(sportKey: string, eventId: string): Promise<EventOdds> {
    const odds = MOCK_EVENT_ODDS.find(e => e.id === eventId);
    if (!odds) {
      // Generar cuotas predeterminadas seguras para cualquier ID desconocido en mock
      const defaultOdds: EventOdds = {
        id: eventId,
        sportKey,
  ...
  async getMatchOddsComparison(
    ...
    return Promise.resolve(OddsNormalizer.buildOddsComparison(event, requestedBookmakers));
  }
}
