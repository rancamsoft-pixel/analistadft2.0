...
export interface NormalizedStoredMatch {
  matchId: string;
...
export class ApiFootballNormalizer {
  static mapStatus(shortStatus?: string): MatchStatus {
    switch (shortStatus) {
      case '1H':
      case '2H':
      case 'HT':
  ...
  static normalizeMatch(raw: any): SportMatch {
  ...
  static toStoredMatch(match: SportMatch): NormalizedStoredMatch {
    return {
      matchId: match.id,
      provider: 'api-football',
  ...
  static normalizeCompetitions(rawList: any[]): Competition[] {
  ...
  static normalizeSeasons(rawList: any[]): Season[] {
    return (rawList || []).map(item => ({
      year: item.year || item,
      start: item.start || '',
      end: item.end || '',
  ...
  static normalizeStandings(competitionId: string, season: string, rawResponse: any): StandingsTable {
    const league = rawResponse?.[0]?.league || {};
    const rawTable = league.standings?.[0] || [];

    const standings: StandingItem[] = rawTable.map((item: any) => ({
      rank: item.rank || 0,
      team: {
  ...
  static normalizeStatistics(rawStats: any[]): MatchStatistics {
    const homeRaw = rawStats?.[0]?.statistics || [];
    const awayRaw = rawStats?.[1]?.statistics || [];

    const getVal = (list: any[], type: string): number => {
      const item = list.find((s: any) => s.type?.toLowerCase() === type.toLowerCase());
  ...
  static normalizeH2H(rawList: any[]): HeadToHead {
    let homeWins = 0;
    let awayWins = 0;
    let draws = 0;
  ...
  static normalizeInjuries(rawList: any[]): InjuryReport[] {
    return (rawList || []).map(item => ({
      player: {
  ...
  static normalizeSquad(rawList: any[]): SquadPlayer[] {
  ...
  static normalizeLineups(rawList: any[]): { home: MatchLineup; away: MatchLineup } | null {
    if (!rawList || rawList.length < 2) return null;

    const mapOne = (raw: any): MatchLineup => ({
      team: {
  ...
  static normalizePredictions(rawItem: any): MatchPrediction | null {
    if (!rawItem || !rawItem.predictions) return null;
