...
export const MOCK_MATCHES: SportMatchDetails[] = [
  {
    id: 'match-101',
...
export class MockSportsProvider implements SportsDataProvider {
  readonly providerName = 'MockSportsProvider';

  async getCompetitions(): Promise<Competition[]> {
    return Promise.resolve(MOCK_COMPETITIONS);
  }

  async getSeasons(_competitionId: string): Promise<Season[]> {
  ...
  async getH2H(_teamAId: string, _teamBId: string): Promise<HeadToHead> {
    return Promise.resolve({
      totalMatches: 8,
      homeWins: 4,
      awayWins: 2,
  ...
  async getLineups(_matchId: string): Promise<{ home: MatchLineup; away: MatchLineup } | null> {
    return Promise.resolve({
      home: {
